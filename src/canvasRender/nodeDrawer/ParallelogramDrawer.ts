...
export default function parallelogramDraw(node: GraphicNode, ctx: CanvasRenderingContext2D): void {
    ...
    const b = new Point(node.x + node.w, node.y);
    const d = new Point(node.x, node.y + node.h);
    const c = new Point(node.x + node.w-delta, node.y + node.h);
